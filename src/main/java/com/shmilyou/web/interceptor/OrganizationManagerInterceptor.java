...
/** 机构管理页面拦截 */
public class OrganizationManagerInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object obj) throws Exception {
        ...
        // 是否登录判断
        if (request.getSession().getAttribute(Constant.LOGIN_ORGANIZATION) == null) {
            //记录下当前url，用于登录后跳转
            ...
            response.sendRedirect(request.getContextPath() + "/phisoa/login/organization");
            return false;
        } else {
            return true;
        }
